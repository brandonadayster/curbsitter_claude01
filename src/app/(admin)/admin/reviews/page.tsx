...
import { decideOrderReview, decideReview } from "./actions";
              ...
              <li key={order.id} className="rounded-2xl border border-border bg-surface p-6">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    ...
                    {schedules.length === 0 || schedules[0]?.weekday === null ? (
                      <p className="mt-1 text-base text-danger">
                        No verified collection day — approving will fail until the schedule is set.
                      </p>
                    ) : null}
                    {hazards.length > 0 ? (
                      <p className="mt-1 text-base">
